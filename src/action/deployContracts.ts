...
import { Address, Hex, createPublicClient, getAddress } from "viem"
...
const deployToChain = async (
  ...
  const kernelAccount = await createEcdsaKernelAccountClient({
    chain: chain.viemChainObject,
  ...
  const result = await publicClient
    .call({
      account: kernelAccount.account.address,
  ...
  const op = await kernelAccount.prepareUserOperationRequest({
    userOperation: {
      callData: await kernelAccount.account.encodeCallData({
        to: DEPLOYER_CONTRACT_ADDRESS,
        data: ensureHex(salt + bytecode.slice(2)),
        value: 0n
      })
  ...
  const opHash = await kernelAccount.sendUserOperation({
    userOperation: op
  })
